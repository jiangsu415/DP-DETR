...
# 使用dist.warp_model方法将配置参数中的模型转移到指定设备上
        self.model = dist.warp_model(cfg.model.to(device), cfg.find_unused_parameters, cfg.sync_bn)
        self.criterion = cfg.criterion.to(device) # 将配置参数中的损失函数转移到指定设备上。
        self.postprocessor = cfg.postprocessor # 设置后处理器
    ...
    def train(self, ):
        self.setup()
        self.optimizer = self.cfg.optimizer # 设置模型的优化器
        self.lr_scheduler = self.cfg.lr_scheduler # 设置模型的学习率调度器

        # NOTE instantiating order 如果配置文件中设置了resume为True，则执行以下操作，这可能意味着从之前的检查点恢复训练
        if self.cfg.resume:
            print(f'Resume checkpoint from {self.cfg.resume}')
            self.resume(self.cfg.resume)
# 将训练数据加载器进行处理，可能是为了支持分布式训练，并根据配置文件中的设置进行相应的操作，如是否打乱数据。
